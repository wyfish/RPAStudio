...
namespace RPA.Integration.Activities.Database
{
    public class DatabaseConnection : IDisposable
    {
        ...
        internal virtual int InsertDataTable(string tableName, DataTable dataTable)
        {
            ...
            {
                SharedObject.Instance.Output(SharedObject.enOutputType.Error, "InsertDataTable Error", e.Message);
                return -1;
            }
        }
        ...
        private void SetupCommand(string sql, Dictionary<string, Tuple<object, ArgumentDirection>> parameters, int commandTimeout, CommandType commandType = CommandType.Text)
        {
            if (_connection == null)
            {
                SharedObject.Instance.Output(SharedObject.enOutputType.Error, "数据库连接为空!");
            }
