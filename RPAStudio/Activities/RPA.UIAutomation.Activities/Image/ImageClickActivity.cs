...
namespace RPA.UIAutomation.Activities.Image
{
    ...
    public sealed class ImageClickActivity : CodeActivity
    {
        ...
        [Browsable(false)]
        public string _DisplayName { get { return "Image Click"; } }
        ...
        protected override void Execute(CodeActivityContext context)
        {
            try
            {
                ...
                var ret = ImageActionCommon.CVMouseAction(SourceImgPath, FlaxCV.CvActionType.LClick, title, matchingThreshold, matchingInterval, retry);
                Result.Set(context, ret.IsMatched);
            }
            catch (Exception e)
            {
                SharedObject.Instance.Output(SharedObject.enOutputType.Error, "Error on Executing ImageClickActivity()", e.Message);
